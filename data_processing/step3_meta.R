...
if(args$exclude) {
  all_exclude <- readr::read_delim(paste0(args$diags_file_path, "_excls.csv"), delim=",") 
...
if(args$calc_all_times) {
  ...
  all_times <- readr::read_delim(paste0(args$res_dir, args$file_name, "_alltimes.csv"))
}
...
readr::write_delim(data %>% dplyr::select(-ABNORM_BIN) %>% filter(!(FINNGENID %in% age_excl$FINNGENID)) %>% dplyr::mutate(DATE=as.Date(DATE)), paste0(args$res_dir, args$file_name, ".csv"), delim=",")