...
args_list <- list(
  ...
  make_option(c("-f", "--file_path_atcs"), action="store", type="character", default="/home/ivm/valid/data/extra_data/data/processed_data/step1/atcs_r12_2025-02-04_min1pct_sum_onttop_2025-02-18.csv",
...
atc_data <- readr::read_delim(args$file_path_atcs)
...
if(args$current) {
  ...
  atc_data <- dplyr::left_join(atc_data, start_dates, by="FINNGENID")
  atc_data <- dplyr::filter(atc_data, DATE>=DATA_START_DATE) %>% dplyr::select(-DATA_START_DATE)
  print(atc_data)
}
# Data in at least 1% of individuals
end_dates <- end_dates %>% dplyr::select(FINNGENID, START_DATE)
atc_data <- dplyr::left_join(atc_data, end_dates, by="FINNGENID")
atc_data <- dplyr::mutate(atc_data, START_DATE=as.Date(START_DATE))
atc_data <- dplyr::filter(atc_data, DATE < START_DATE)

# Count
atc_wider <- atc_data %>% dplyr::group_by(FINNGENID, ATC_FIVE) %>% dplyr::reframe(N_ATC=n()) %>% dplyr::ungroup()
if(args$bin_count == 1) {
  atc_wider <- atc_wider %>% dplyr::mutate(N_ATC=ifelse(N_ATC>0, 1, 0))
}
atc_wider <- atc_wider %>% tidyr::pivot_wider(names_from=ATC_FIVE, values_from=N_ATC, values_fill=0)

## Adding info on when was last recorded ATC-code
last_date <- dplyr::select(atc_data, FINNGENID, DATE) %>% distinct() %>% dplyr::group_by(FINNGENID) %>% dplyr::arrange(FINNGENID, desc(DATE))
last_date <- last_date %>% slice_min(order_by=desc(DATE))
atc_wider <- dplyr::left_join(atc_wider, last_date)
atc_wider <- dplyr::rename(atc_wider, LAST_ATC_DATE=DATE)
print(atc_wider)
dir.create(args$res_dir, showWarnings=FALSE, recursive = TRUE)

out_file_path <- ""
if(args$out_file_name != "") {
  out_file_path <- paste0(args$res_dir, "atcs_", args$out_file_name, "_", args$file_name, ".csv")
} else {
  out_file_path <- paste0(args$res_dir, "atcs_", args$file_name, ".csv")
}
readr::write_delim(atc_wider, out_file_path, delim=",")
