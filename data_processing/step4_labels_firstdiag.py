...
def get_controls(data, months_buffer=0):
    ...
    # only keeping those whose last value was normal -> worried that single abnormal in the end is due to LOF
    remove_fids = controls.loc[controls.LAST_ABNROM>0, "FINNGENID"]
    controls = controls.loc[controls.LAST_ABNROM==0].copy()
...
if __name__ == "__main__":
    ...
    controls, remove_fids = get_controls(data, args.months_buffer)
    logging.info("Removed " + str(len(remove_fids)) + " individuals due to potential loss of followup")
    pd.DataFrame({"FINNGENID":list(set(remove_fids))}).to_csv(args.res_dir + "logs/" + args.file_name + "_data-diag_" + get_date() + "_removed_lof_fids.csv", sep=",")
    ...
    print(labels)
    print(f"N rows {len(new_data)}   N indvs {len(labels)}  N cases {sum(labels.y_DIAG)} pct cases {round(sum(labels.y_DIAG)/len(labels), 2)*100}%")
